# [mcmc2bugs(),as.bugs.array2(),.decode.parameter.name()] can transform mcmc list to bugs output format
# [mcmc2bugs(),as.bugs.array2(),.decode.parameter.name()] are copied from R2jags package; https://github.com/cran/R2jags/tree/master/R
# xiao_bugsjags() works similar as R2jags::jags, but is expected to be more flexible.
...
xiao_bugsjags <- function(data= WinBUGS.data,
                          inits=WinBUGS.inits,
  ...
  Jmodel <- rjags::jags.model(model.file,
                              data = data,
                              inits= inits,
                              n.chains=n.chains,
                              n.adapt=100)
  update(Jmodel, n.iter=n.burnin)
  
  jags_samples <- rjags::coda.samples(model=Jmodel,
  ...
  fit <- mcmc2bugs(jags_samples, model.file = model.file, program = "jags", 
                   DIC = DIC, DICOutput = NULL, 
                   n.iter = n.iter, n.burnin = 0, 
                   n.thin = n.thin)
  out <- list(model = Jmodel, BUGSoutput = fit,mcmc.list = jags_samples,
